...
<!--            <th class="title">{{ formatNumber(sumField("deathsPerOneMillion")) }}</th>-->
        ...
        <template v-slot:item="{ item }">
          <tr>
            <td>
              <v-img :src="item.countryInfo.flag" :alt="item.country" width="30px"></v-img>
            </td>
...
<script>
import CountryYesterdayService from "@/services/CountryYesterdayService";
...
export default {
  ...
  mounted() {
    CountryYesterdayService.getCountries().then(response => {
